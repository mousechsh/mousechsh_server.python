...
__version__ = "1.0"
__all__ = ["MouseChshHexStringBytes"]
...
# left: bytes from HexString, right: bytes
class MouseChshHexStringBytes(MouseChshCodec):
	...
	def from_right_to_left(self):
		try:
			data = base64.b16encode(self.right())
			return self.__utf8bytes.right__(data).left()
		except Exception as ex:
			mousechsh_logging_exception(ex, '二进制字符串转为十六进制字符串时遇到错误：')
	...
	def from_left_to_right(self):
		try:
			...
			mousechsh_logging_exception(ex, '十六进制字符串转为二进制字符串时遇到错误：')
