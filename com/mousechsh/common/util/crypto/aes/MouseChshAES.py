...
__version__ = "1.0"
__all__ = ["MouseChshAESpECBpPKCS5"]
...
class MouseChshAESpECBpPKCS5(MouseChshCrypto):
	...
	def byte_len(self, byte_len=None):
		...
		if byte_len == 16:  # = 128 / 8
			super().byte_len(16)
		elif byte_len == 24:  # = 192 / 8
			super().byte_len(24)
		elif byte_len == 32:  # = 256 / 8
			super().byte_len(32)
		else:  # = 128 / 8
			super().byte_len(16)
	...
	def encrypt(self):
		if not isinstance(self.plaintext(), bytes):
			...
			return
		try:
			cryptor = AES.new(key=self.key(), mode=self.mode())

			data = self.padding().content__(self.plaintext()).data()
			data = cryptor.encrypt(data)
			self.ciphertext(data)
		except Exception as ex:
			mousechsh_logging_exception(ex, '加密编码AES/ECB/PKCS5Padding时遇到错误：')
			self.ciphertext(self.default_data())
	...
	def decrypt(self):
		...
		try:
			...
			mousechsh_logging_exception(ex, '解密解码AES/ECB/PKCS5Padding时遇到错误：')
			self.plaintext(self.default_data())
