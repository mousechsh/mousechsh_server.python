...
__version__ = "1.0"
__all__ = ["MouseChshWebSocketServerCmd"]
...
from com.mousechsh.common.middle.ws.MouseChshWebSocketServer import mousechsh_websocket_server_runner
from com.mousechsh.common.middle.ws.MouseChshWssServer import mousechsh_wss_server_runner


@mousechsh_cmd_annotation()
class MouseChshWebSocketServerCmd(MouseChshCmd):
	...
	def run(self):
		try:
			if self.__ssl:
				self.set_object(mousechsh_wss_server_runner(
					self.__host, self.__port,
					pk_path=self.__pk_path, key_path=self.__key_path
				))
			else:
				self.set_object(mousechsh_websocket_server_runner(self.__host, self.__port))
