...
__version__ = "1.0"
__all__ = ["MouseChshHttpServerCmd"]
...
from com.mousechsh.common.middle.http.MouseChshHttpServer import mousechsh_http_server_runner
from com.mousechsh.common.middle.http.MouseChshHttpsServer import mousechsh_https_server_runner


@mousechsh_cmd_annotation()
class MouseChshHttpServerCmd(MouseChshCmd):
	...
	def run(self):
		try:
			if self.__ssl:
				self.set_object(mousechsh_https_server_runner(
					self.__host, self.__port,
					pk_path=self.__pk_path, key_path=self.__key_path
				))
			else:
				self.set_object(mousechsh_http_server_runner(self.__host, self.__port))
